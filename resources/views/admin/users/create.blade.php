...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.users.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('jobtile_id') ? 'has-error' : '' }}">
                <label for="jobtile">{{ trans('cruds.user.fields.jobtile') }}*</label>
                <select name="jobtile_id" id="jobtile" class="form-control select2" required>
                    @foreach($jobtiles as $id => $jobtile)
                        <option value="{{ $id }}" {{ (isset($user) && $user->jobtile ? $user->jobtile->id : old('jobtile_id')) == $id ? 'selected' : '' }}>{{ $jobtile }}</option>
                    @endforeach
                </select>
                @if($errors->has('jobtile_id'))
                    <em class="invalid-feedback">
                        {{ $errors->first('jobtile_id') }}
                    </em>
                @endif
            </div>
